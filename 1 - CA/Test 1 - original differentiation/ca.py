...
class ca:
	...
	def states(self,y,x,checkSpace):
		...
		if (checkSpace[y][x]!=0):
			...
			neighb=self.checkNeighbourhood(y,x)
			ch=[]
			if (checkSpace[y][x] in neighb) and (self.n > 4) : #if a neighbour has the same function then differentiate
				for f in self.functions:
					...
					# print ch
				if self.choice==0:
					self.space[y][x]=random.choice(ch) #differentiate to random
				#elif self.choice==1:
					#100%= pure diversity
					#0% = homogoneity
