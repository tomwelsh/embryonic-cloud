...
class cell:
	...
	def __init__(self,motherPort,f,divs,maxsubs,time0,time1,hostPort=None):
		self.maxsubs=int(maxsubs)
		...
		self.badThresh=int(time0)
		self.badChances=int(time1)
		self.node=node(hostPort,self,self.genome,self.maxsubs,[self.badThresh,self.badChances])
		...
		for i in range(int(self.maxDivisions)):
			self.divide(f,self.maxDivisions/2,self.maxsubs)
		#self.startCellThread()
		#if mother==True:
		#	self.startAPIThread()
	#	print("A")
	...
	#divide techniques:
		...
		if self.divisions > 0:
			...
			os.system("python3 blankCell.py %s %d %d %d %d %d  &" % (str(self.node.nodeId), f, divs,subs,self.badThresh,self.badChances))
			time.sleep(5)
	...
	def refreshLocalNodes(self):
		with open('nodes'+str(self.node.pubPort),'r') as nFile:
				for line in nFile.readlines():
					node=(line.rstrip('\n'))
					if ((node != self.node.nodeId) & (node not in self.node.localNodes) & (node not in self.node.badNodes)):
...
#		self.startAPIThread()
		...
		while(1):
			...
			if ((int(time.time()-self.sTime)) % 30) == 0:
...
class node:
	...
	def __init__(self,port,cell,genome,maxnodes,timeo):
		...
		self.badThreshold=timeo[0]
		self.badChances=timeo[1]
	...
	def nodeSubscribe(self,nId,div=False):
		...
		self.cell.lock.acquire()
		if div==True:					#if it a child cell			##fix meeee
			if(nId != self.nodeId):
				try:
			...
			if len(list(self.localNodes.keys())) < self.maxNodes:
				#print(len(list(self.localNodes.keys())))
	...
	def nodeUnsubscribe(self,nId):
		...
		if self.nodeData[nId][3]==True:
			...
			self.cell.divide(self.genome.maxfunc,0,self.maxNodes)
	...
	#	print msgs
		...
		if msg['typ'][0]=="A":
			...
			#TODO!!!!
			#here we must include a check against the message, if it is malformed in anyway it must be reported as a bad cell
	...
	def checkTimeouts(self):
		...
		for n in list(self.localNodes.keys()):
			...
			##print(timeSince)
			##print(timeSince)lse
				if timeSince > self.badThreshold:
					self.nodeData[n][1]=time.time() #set last timeout
					self.nodeData[n][2]+=1
					if self.nodeData[n][2] >= self.badChances:
						self.badNodes.append(n)
						self.debug('out', json.dumps({'typ':'seen timeout','node':n}))
						#print("seen")
						#print(n)
						#print(self.cell.motherPort)
						if n == (self.cell.motherPort):
						#	print('suicide!')
							exit()
						self.nodeUnsubscribe(n)
